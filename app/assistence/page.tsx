...
export default function Assistence() {
    ...
    const switchStatus = (x: keyof IStatusSection) => {
        console.log(`Tentativo di attivare la sezione prima: ${x}`);
        chageStatus({
            buttonSection: false,
            emailSection: false,
            ticketSection: false,
    ...
    useEffect(() => {
        // Questa funzione viene eseguita solo dopo il re-render, quindi lo stato è aggiornato qui
        ...
        <main>
                    ...
                    <section className=" flex h-screen items-center justify-center flex-col ">
                        ...
                        <div className=" w-full lg:w-fit h-auto flex item-center justify-around">
                            <button className="m-2 px-8 py-2 md:px-20 md:py-4 md:text-3xl lg:text-2xl lg:px-15 lg:py-2 bg-gray-600 text-white font-semibold rounded-md shadow-md hover:bg-gray-700 hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-opacity-75 transition-all cursor-pointer"
                                onClick={() => switchStatus('emailSection')}
                            >Customer</button>
                            <button className="m-2 px-8 py-2 md:px-20 md:py-4 md:text-3xl lg:text-2xl lg:px-15 lg:py-2 bg-indigo-600 text-white font-semibold rounded-md shadow-md hover:bg-indigo-700 hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-opacity-75 transition-all cursor-pointer"
                                onClick={() => switchStatus('ticketSection')}
                            >Assistence</button>
                        </div>
                    </section>
                )
            ...
            {statusSection.emailSection &&  (
                <section className=" flex h-screen items-center justify-center flex-col ">
                    ...
                    <button className="my-2 text-gray-300 border-b-2 border-t-2 border-gray-300 py-1 transition-all hover:py-2" onClick={() => switchStatus('buttonSection')}>Torna indietro</button>
                </section>
            )}

            {statusSection.ticketSection && (
                <section className=" flex h-screen items-center justify-center flex-col p-3">
                    <div className="h-full w-full bg-[#161616] rounded-lg overflow-hidden overflow-y-auto">
                        <Ticket onNavigateBack={() => switchStatus('buttonSection') }/>
                    </div>
                </section>
            )}
        </main>
